...
def calculate_confusion_matrix(y_true, y_pred, num_classes):
    ...
    for i in range(len(y_pred)):
        row_index = int(y_pred[i]) - 1  # 예측값의 인덱스를 0부터 시작하도록 조정
        col_index = int(y_true[i]) - 1  # 실제값의 인덱스를 0부터 시작하도록 조정
...
# 예측 결과 계산
y_train_pred = np.argmax(y_hat_train, axis=0) + 1
y_test_pred = np.argmax(y_hat_test, axis=0) + 1

# Confusion Matrix 계
test_confusion_matrix = calculate_confusion_matrix(y_test.flatten(), y_test_pred, num_classes=6)
