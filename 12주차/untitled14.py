...
for epoch in range(epochs):
    ...
    A_train, b_train, b_with_dummy_train, B_train, y_hat_train = forward_propagation(x_train_with_dummy, best_v, best_w)
    ...
    mse_test = np.mean((y_hat_test - y_test_one_hot.T)**2)
    mse_list.append(mse)
...
plt.plot(range(1, epochs+1), mse_test_list, label='Train MSE', color='blue')
plt.xlabel('Epochs')
plt.ylabel('MSE')
