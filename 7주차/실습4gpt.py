...
# 선형 기저 함수 모델의 가중치 계산 함수 정의
def calculate_weights(X, Y, K):
    X_b = np.column_stack([gaussian_basis_function(X, K, k) for k in range(K)])  # 선형 기저 함수로 변환
    weights = np.linalg.inv(X_b.T @ X_b) @ X_b.T @ Y  # 가중치 계산
    return weights
