...
# 훈련 데이터에 대한 가중치 계산
train_weights = calculate_weights(train_set[:, 0], train_set[:, 1], 70)

for K in K_values:
    # 훈련 데이터에 대한 MSE 계산 및 저장
    train_mse = mse(train_set[:, 0], train_set[:, 1], K, train_weights)
    train_mse_values.append(train_mse)
    
    # 테스트 데이터에 대한 MSE 계산 및 저장
    test_mse = mse(test_set[:, 0], test_set[:, 1], K, train_weights)
...
# 그래프 그리기
plt.figure(figsize=(10, 6))
plt.plot(K_values, train_mse_values, label='Training MSE', marker='o')
plt.plot(K_values, test_mse_values, label='Test MSE', marker='o')
