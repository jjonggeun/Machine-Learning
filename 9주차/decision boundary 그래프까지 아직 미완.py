...
# Train set과 test set에 대한 결정 경계 그리기
plt.figure(figsize=(10, 6))

# Train set에 대한 결정 경계
x1_min, x1_max = train_set[:, 0].min(), train_set[:, 0].max()
x2_min, x2_max = train_set[:, 1].min(), train_set[:, 1].max()
xx1, xx2 = np.meshgrid(np.linspace(x1_min, x1_max), np.linspace(x2_min, x2_max))
X_mesh = np.c_[xx1.ravel(), xx2.ravel(), np.ones(xx1.ravel().shape)]
Z_train = predict(X_mesh, np.array([w0_history[-1], w1_history[-1], w2_history[-1]]))
Z_train = Z_train.reshape(xx1.shape)
plt.contourf(xx1, xx2, Z_train, alpha=0.3, cmap='coolwarm')
plt.scatter(train_set[:, 0], train_set[:, 1], c=train_set[:, 2], cmap='coolwarm', edgecolors='k', label='Train Set')

# Test set에 대한 결정 경계
Z_test = predict(X_mesh, np.array([w0_history[-1], w1_history[-1], w2_history[-1]]))
Z_test = Z_test.reshape(xx1.shape)
plt.contour(xx1, xx2, Z_test, levels=[0.5], colors='k', linewidths=1)
plt.scatter(test_set[:, 0], test_set[:, 1], c=test_set[:, 2], cmap='coolwarm', marker='x', label='Test Set')
