...
#     dataset[:, 0] = np.where(dataset[:, 0] == 'female', 0, 1)
    
#     # a neurological disorder: no -> 0, yes -> 1
#     dataset[:, 4] = np.where(dataset[:, 4] == 'no', 0, 1)
...
def select_features(directory):
    ...
    # CSV 파일에서 첫 번째 줄을 읽어 열 이름을 추출
    with open(path, 'r') as file:
        column_names = file.readline().strip().split(',')
    
    # 열 이름을 기준으로 인덱스 찾기
    gender_idx = column_names.index("gender")
    neuro_idx = column_names.index("a neurological disorder")
    target_idx = column_names.index("heart disease")
    ...
    # a neurological disorder: no -> 0, yes -> 1
    dataset[:, neuro_idx] = np.where(dataset[:, neuro_idx] == 'no', 0, 1)
