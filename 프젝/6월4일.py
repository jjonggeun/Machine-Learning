...
def select_features(directory):
    ...
    # 특징 추출 (상관관계에 기반하여 조합)
    feature_1 = dataset[:, chol_idx] * dataset[:, age_idx]  # 콜레스테롤과 나이의 곱
    feature_2 = (dataset[:, hbp_idx] + 1) / (dataset[:, bloodsugar_idx] + 1)  # 고혈압과 혈당 수치의 비율
    feature_3 = np.log(dataset[:, age_idx] + 1) * dataset[:, height_idx]  # 나이와 키의 조합
    feature_4 = dataset[:, weight_idx] / (dataset[:, height_idx] + 1)  # 체중과 키의 비율
    feature_5 = dataset[:, weight_idx] / ((dataset[:, height_idx]/100)**2)
    feature_6 = dataset[:, height_idx]
    
    #표준화 : 
    # 선택한 특징들을 결합
    selected_features = feature_6
