...
def select_features(file_path):
    ...
    feature_4 = balanced_data[:, weight_idx] / balanced_data[:, age_idx]  # 몸무게 나이의 비율
    ...
    # 선택한 특징들을 결합
    selected_features = np.column_stack((feature_1,feature_4,feature_5))
    # 특징 1개
...
v = np.random.randn(hidden_size, M + 1)*0.005
w = np.random.randn(output_size, hidden_size + 1) *0.005
