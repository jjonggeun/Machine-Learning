...
def k_means_clustering(data, K):
    np.random.seed(42)  # 결과 재현성을 위해 시드 설정
    ...
    for i in range(1, K):
        distances = np.min(np.array([np.linalg.norm(data - center, axis=1) for center in centers[:i]]), axis=0)
...
# 원하는 K 값 설정
K = 2 # 클러스터의 개수를 설정합니다. 이 값을 변경하여 다른 K 값을 테스트할 수 있습니다.
